...
#define LOW_HALF_BYTE(byte) \
  (static_cast<std::uint8_t>(MASK_LOW_HALF_BYTE(byte)))
#define HIGH_HALF_BYTE(byte) (LOW_HALF_BYTE(byte >> 4))

#define LOW_BYTE(word) (static_cast<std::uint8_t>(MASK_LOW_BYTE(word)))
#define HIGH_BYTE(word) (LOW_BYTE(word >> 8))

#define LOW_WORD(dword) (static_cast<std::uint16_t>(MASK_LOW_WORD(dword)))
